...
import pytz
from tradingview_ta import TA_Handler, Interval, Exchange # 引入新库
...
def update_history(data):
    ...
    # 这里的 CN10Y 现在可以直接从 data 里取到了
    cn10y_val = data.get('CN10Y', "")
    if cn10y_val == "Link": cn10y_val = ""

    new_row = {
        "日期": today,
        "US10Y": get_val('US10Y'),
        "CN10Y": cn10y_val, # <--- 存入 TradingView 数据
        "DXY": get_val('DXY'),
        "RRP": get_val('RRP'),
        "VIX": get_val('VIX'),
        "HYG": get_val('HYG'),
...
def generate_html(current_data, history_df):
    ...
    links = {
        "CN10Y": "https://www.tradingview.com/symbols/TVC-CN10Y/",
    ...
    # RRP/TGA 显示逻辑
    rrp_cell = cell(current_data.get('RRP', 'Link'), links['RRP'])
    tga_cell = cell(current_data.get('TGA', 'Link'), links['TGA'])
